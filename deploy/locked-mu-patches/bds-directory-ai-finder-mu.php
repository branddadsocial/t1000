...
define( 'BDS_AI_FINDER_VER', '1.11.6' ); // #50 no-markdown prompt rule + #61 Enter submits chat
...
function bds_ai_finder_match_services( $message, $parsed ) {
	...
	$catalog = array(
		...
		array(
			...
			'blurb'   => 'Local & web (GBP, SEO, speed, care), AI Ads, and social growth packages. Clear scoped deliverables.',
...
function bds_ai_finder_openai_reply( $key, $message, $history, $listings, $services, $guest, $parsed, $sponsored = array(), $products = array(), $need = array(), $clarify = array() ) {
	...
	$system = "You are Ask BrandDad - a helpful person on the BrandDad team who knows directory.branddad.social.\n"
		...
		. "- Write plain sentences. Do NOT use markdown link syntax like [name](url) and do not paste raw URLs into the reply — the interface prints that text literally. Name a business by its name; its card and link are rendered separately.\n"
		. "- " . $guest_rule . "\n"
...
(function(){
  ...
  function boot(){
    ...
    // Enter submits (some themes swallow the native single-input submit). preventDefault
    // then one requestSubmit -> exactly one submit whether or not native fires.
