 ...
 * Author: BrandDad
 *
 * 1.11.4 ΓÇö Dark-theme contrast for /services/ lanes (#lane-local) ΓÇö navy text
 *         on dark chrome was unreadable. Preserve layout; invert tokens only.
 * 1.11.3 ΓÇö Authority tighten: money PDPs on home, converting sister doors
 *         (BrandDad.co /get-started/, HostTech /shopping/), Partnero on strips,
 *         /services/ primary CTA stays on catalog. Shop still OUT.
 * 1.11.2 ΓÇö Front page always expands [branddad_home_hub] (Elementor/cache leftover).
 * 1.11.1 ΓÇö Learn dropdown keeps Explained / Guides / Courses / Playbooks. Shop still OUT.
 * 1.11.0 ΓÇö Home ΓÇ£Check your websiteΓÇ¥ ΓåÆ dedicated /check-your-website/ tool page.
 * 1.10.10 ΓÇö Learn dropdown: Learning Center / Explained / Guides / Courses / Playbooks. Shop still OUT.
 * 1.10.9 ΓÇö Home ΓÇ£Check your websiteΓÇ¥ ΓåÆ /?bds_health=1 (pretty path was 301ing to a bare home).
 * 1.10.8 ΓÇö Learn dropdown: Learning Center / Books / Courses. Shop still OUT of header.
 * 1.10.7 ΓÇö SMM hub: monthly $175 slug unchanged; +Video $355 sibling. Partnero kept.
 * 1.10.6 ΓÇö Partnero on home ecosystem doors; Social stays services/learning (not a logo factory).
 * 1.10.5 ΓÇö Explicit Home in primary header (logo alone is not enough).
 *         Shop stays OUT. Learn / Cart / Account / List stay.
 * 1.10.4 ΓÇö Shop stays OUT of primary header (sibling chrome 1.2.1 had re-added it).
 *         Learn / Cart / Account stay. Catalog via /services/ cards, PDPs, footer.
 * 1.10.3 ΓÇö Compact converting header: drop redundant Shop from primary nav
 *         (catalog stays on /services/ cards, PDPs, footer, /shop/ URL).
 *         Partnero on List CTA via add_query_arg; Home dropped (logo).
 * 1.10.2 ΓÇö Soft-path CTAs: /sign-in /login ΓåÆ Woo My Account; /register ΓåÆ
 *         Directory registration; /ai-ads ΓåÆ services #lane-ai when AI Ads
 *         rewrite is cold; nav adds Learning Center / Cart / Account;
 *         Partnero kept; photo cards from 1.10.0 unchanged.
 * 1.10.1 ΓÇö /services/ CTA wiring: #lane-local panel ΓåÆ Woo My Account (Social
 *         /ai-network/ 301'd home); quiz Google/search ΓåÆ #lane-local; unwrap
 *         AALinks keyword autolinks; Partnero on quiz + explainer URLs.
 * 1.10.0 ΓÇö Home + /services/ listing cards: restore tasteful AI product photos
 *         when attachment has _bds_svc_ai_photo; skip GD covers/placeholders;
 *         Partnero kept; no outreach; AI Ads pricing lane unchanged.
 * 1.9.9 ΓÇö /services/ #lane-local: compact site/social check strip ΓåÆ Health Check or
 *         member panel tools; Partnero ref kept; no outreach; no clutter.
 * 1.9.8 ΓÇö /services/ hero + lane CTAs: fix ghost/secondary contrast (white-on-white
 *         from .bdsu-primary color:!important); CTA fills #0260d9 + white (ΓëÑ4.5:1).
 * 1.9.7 ΓÇö Home + /services/ listing cards: text-only (no product photos/plates) for
 *         uniform conversion; AI Ads tier lane kept; Partnero kept; no outreach.
 * 1.9.6 ΓÇö /services/ #lane-ai: SetupΓåÆStarterΓåÆGrowthΓåÆScale pricing lane.
 * 1.9.5 ΓÇö ΓÇ£Check your websiteΓÇ¥ Health Check entry on home + /services/; Partnero kept.
 * 1.9.4 ΓÇö Home popular-services block: converting headline/subcopy, shop layout,
 *         stronger card CTAs; Partnero ref kept; no outreach; catalog unchanged.
 * 1.9.3 ΓÇö /services/ conversion layout: problemΓåÆpathsΓåÆfeatured Local/Web + AI AdsΓåÆ
 *         more growth; network doors deferred; Partnero ref kept; no outreach.
 * 1.9.2 ΓÇö catalog Local & Web (13) + AI Ads (5) + growth; Partnero; no outreach.
 */
...
define( 'BDSU_VER', '1.11.4' );

/** BrandDad support WhatsApp ΓÇö restore if a privacy buffer rewrote it to registration. */
...
/**
 * Member Account Control ΓÇö site health / SEO / social tools panel.
 *
 * @return string
 */
function bdsu_panel_tools_url() {
	// Social member tools live on Woo My Account (Health MU dashboard section).
	// /ai-network/ is Directory Account Control ΓÇö on Social it 301s to home.
 ...
 * No separate public social-profile scanner ΓÇö panel tools cover site + social guidance.
 *
 * @return string
 */
function bdsu_lane_local_check_html() {
	...
	<aside class="bdsu-lane-check" id="lane-local-check" aria-label="Check your website or open panel tools">
		<div class="bdsu-lane-check__copy">
			...
			<span>Run a free public-signal site check ΓÇö results point to SEO Audit, Speed, GBP, and other Local &amp; Web services when relevant. Prefer your member panel for site + social tools.</span>
		</div>
...
/** Compact BrandDad network doors ΓÇö used on /services and home (not in hero). */
function bdsu_network_doors_html( $context = 'services' ) {
	...
	$doors = array(
		array(
			...
			'blurb' => 'Logos and premium websites live on BrandDad.co ΓÇö not here.',
			'cta'   => 'Start a logo or website ΓåÆ',
			'url'   => bdsu_po_keep( 'https://branddad.co/get-started/?' . $utm ),
		),
		array(
			'label' => 'Need hosting?',
			'blurb' => 'Domains and hosting live on HostTech ΓÇö where the site lives.',
			'cta'   => 'Shop HostTech plans ΓåÆ',
	...
	<section class="bdsu-network-doors" id="bdNetworkDoors">
		<div class="bdsu-section-intro">
			...
			<p>Same family. Pick the door that fits ΓÇö logos, hosting, or Directory savings.</p>
		</div>
...
function bdsu_money_pdp_strip_html( $context = 'services' ) {
	...
	$items = array(
		array( 'LinkedIn Visibility', 'Monthly professional authority ΓÇö built around professional visibility.', $p( 'linkedin-visibility-amplification-system-for-professionals' ) ),
		array( 'LinkedIn Content', 'Written posts for credibility, not gimmicks.', $p( 'linkedin-viral-posts-for-professionals' ) ),
		array( 'Instagram Growth', 'Discovery and engagement. No fake followers.', $p( 'instagram-viral-growth-discovery-system' ) ),
		array( 'Telegram Growth', 'Grow and activate a real community.', $p( 'telegram-growth-engagement-system' ) ),
		array( 'Facebook Visibility', 'Scoped campaigns ΓÇö not fake page likes.', $p( 'facebook-growth-visibility-campaigns' ) ),
	);
	ob_start();
	?>
	<section class="bdsu-money-strip" id="bdsu-growth-systems" data-bdsu-context="<?php echo esc_attr( $context ); ?>">
		<div class="bdsu-section-intro">
			...
			<h2>LinkedIn, Instagram, Telegram, Facebook ΓÇö buy the bottleneck.</h2>
		</div>
...
function bdsu_logo_alt( $attr, $attachment ) {
	if ( 171 === (int) $attachment->ID || false !== stripos( (string) ( $attr['alt'] ?? '' ), 'Bing' ) ) {
		$attr['alt'] = 'BrandDad Social ΓÇö business growth, marketing, PR and SEO';
	}
	return $attr;
}
...
function bdsu_styles() {
	...
	';
	wp_register_style( 'bdsu-brand', false, array(), '1.10.6' );
...
function bdsu_member_strip() {
	...
	echo '<div class="bdsu-member-strip">BrandDad Directory members save 10% on eligible BrandDad Social services. <a href="' . esc_url( $join ) . '">Join the Directory ΓåÆ</a></div>';
}
...
function bdsu_clean_output( $html ) {
	...
	// inside hub copy into the Visibility PDP ΓÇö wrong destination from AI Ads / lane copy.
...
/**
 * AI Ads converting tier lane ΓÇö Setup ΓåÆ Starter ΓåÆ Growth ΓåÆ Scale (+ Extra add-on).
 *
 * @return string
 */
function bdsu_ai_ads_lane_html() {
	...
	if ( $extra ) {
		...
		$html  .= '<aside class="bdsu-ai-addon"><div><strong>Need more spend capacity?</strong><span>Extra Spend adds +$1,000/mo managed capacity for $99/mo. Stackable. Your ad spend stays on the platforms.</span></div><a href="' . esc_url( $ex_url ) . '">Add Extra Spend ΓåÆ</a></aside>';
	}
	$html .= '<p class="bdsu-ai-note">Management fees are BrandDad pricing. Ad spend is paid by you directly to Meta / Google / etc. Clear scope.</p>';
	return $html;
}
...
/**
 * Canonical /services/ hub catalog ΓÇö Local & Web (13) + AI Ads (5) + growth lanes.
 * LinkedIn outreach / professional outreach SKUs are intentionally omitted.
 *
 * @return array<int,array{0:string,1:string,2:string,3:string}>
 */
function bdsu_services_catalog_rows() {
	return array(
		// ΓÇöΓÇö Local & Web (13) ΓÇöΓÇö
		array( 'gbp-setup-optimization', 'GBP Setup & Optimization', 'Claim-ready Google Business Profile fields, categories, and consistency checks ΓÇö no ranking promises.', 'Local & Web' ),
		...
		array( 'fix-my-website', 'Fix My Website', 'Scoped website repairs starting at $49. Secure intake for access ΓÇö never paste passwords into normal forms.', 'Local & Web' ),
		...
		array( 'local-seo-management-starter', 'Local SEO ΓÇö Starter', '$199/mo GBP hygiene, citations cadence, and monthly reporting. Real reviews only.', 'Local & Web' ),
		array( 'local-seo-management-growth', 'Local SEO ΓÇö Growth', '$349/mo Starter plus local content support and deeper citation work.', 'Local & Web' ),
		array( 'google-review-growth-setup', 'Review Growth ΓÇö Setup', 'Ethical ask-flow so real customers leave reviews. We never filter ΓÇ£only happyΓÇ¥ reviews to Google.', 'Local & Web' ),
		array( 'google-review-growth-monthly', 'Review Growth ΓÇö Monthly', '$79/mo monitoring and gentle ask cadence. Real reviews only.', 'Local & Web' ),
		array( 'business-directory-distribution', 'Directory Distribution', 'Submit and claim across relevant directories. BrandDad Directory is part of the mix ΓÇö not the only listing.', 'Local & Web' ),
		array( 'website-conversion-makeover', 'Conversion Makeover', 'Clarity, CTA, trust, and form-path improvements ΓÇö not a full redesign (see BrandDad.co for builds).', 'Local & Web' ),
		...
		array( 'monthly-website-care', 'Monthly Website Care', '$69/mo updates, uptime peek, and minor fixes (Γëñ30 min). Larger work is quoted or Fix My Website.', 'Local & Web' ),
		// ΓÇöΓÇö AI Ads (5) ΓÇöΓÇö
		...
		// ΓÇöΓÇö Growth / social / PR (no LinkedIn outreach) ΓÇöΓÇö
...
function bdsu_services_hub() {
	...
	ob_start();
	?>
	<div class="bdsu-services-page bdsu-services-convert" data-bdsu-hub="1.11.4">
		<section class="bdsu-services-hero">
			...
			<p>Pick the bottleneck. Buy a scoped fix with transparent starting prices ΓÇö local &amp; web, AI-managed ads, or growth systems. Clear scope and transparent starting prices.</p>
			...
			<div class="bdsu-trust-row"><span>Γ£ô Free public-signal site check</span><span>Γ£ô Directory members: 10% off eligible services</span><span>Γ£ô WhatsApp before you buy</span></div>
		</section>

		<section class="bdsu-outcome-row" aria-label="Choose your path">
			<a href="<?php echo esc_url( bdsu_site_check_url() ); ?>"><strong>0 ┬╖ Check your site</strong><span>Free explainable score ΓåÆ honest service suggestions</span><b>Run site check ΓåÆ</b></a>
			<a href="#lane-local" data-bdsu-jump="lane-local"><strong>1 ┬╖ Fix the foundation</strong><span>GBP, SEO, speed, reviews, site care</span><b>Browse Local &amp; Web ΓåÆ</b></a>
			<a href="#lane-ai" data-bdsu-jump="lane-ai"><strong>2 ┬╖ Run paid ads</strong><span>Setup ΓåÆ Starter ΓåÆ Growth ΓåÆ Scale</span><b>Browse AI Ads ΓåÆ</b></a>
			<a href="#lane-more" data-bdsu-jump="lane-more"><strong>3 ┬╖ Grow visibility</strong><span>Social, LinkedIn, PR, reputation</span><b>Browse growth ΓåÆ</b></a>
		</section>
		...
		<section class="bdsu-sales-proof" aria-label="Why BrandDad">
			<div><strong>Scoped offers</strong><span>Each product states deliverables ΓÇö not open-ended agency fluff.</span></div>
		...
		<section id="lane-local" class="bdsu-service-section bdsu-lane">
			<div class="bdsu-section-heading">
				<div>
					...
					<h2>Local &amp; Web</h2>
					<p>Foundation fixes for Google Business Profile, SEO, speed, reviews, and ongoing care ΓÇö 13 clear options.</p>
				</div>
				<a href="https://wa.me/18729105115?text=Help%20me%20choose%20a%20Local%20%26%20Web%20service">Ask which one ΓåÆ</a>
			</div>
		...
		<section id="lane-ai" class="bdsu-service-section bdsu-lane bdsu-lane--ai">
			<div class="bdsu-section-heading">
				<div>
					...
					<h2>AI Ads ΓÇö clear tiers, one CTA each</h2>
					<p>Start with Setup once. Then pick the monthly plan that matches your ad spend. Ad spend stays on the platforms ΓÇö you only pay BrandDad the management fee.</p>
				</div>
				<a href="<?php echo esc_url( bdsu_po_keep( home_url( '/ai-ads/' ) ) ); ?>">How AI Ads works ΓåÆ</a>
			</div>
		...
		<section id="lane-more" class="bdsu-service-section bdsu-lane">
			<div class="bdsu-section-heading">
				<div>
					...
					<p>Filter to compare. We do <strong>not</strong> sell professional-outreach campaigns.</p>
				</div>
		...
		<section class="bdsu-after-purchase">
			<div class="bdsu-section-intro"><span>What happens after you buy</span><h2>Checkout ΓåÆ details ΓåÆ delivery</h2></div>
		...
		<section class="bdsu-service-faq">
			...
			<div class="bdsu-faq-list">
				...
				<details><summary>Do you sell LinkedIn professional outreach?</summary><p>No. We offer LinkedIn visibility and content ΓÇö built around professional visibility campaigns.</p></details>
		...
		<section class="bdsu-directory-cta">
			...
			<a href="<?php echo esc_url( bdsu_po_keep( 'https://directory.branddad.social/registration/?utm_source=branddad_social&utm_medium=services&utm_campaign=directory10' ) ); ?>">Join the Directory ΓÇö 10% off</a>
		</section>
...
function bdsu_services_script() {
	...
	wp_register_script( 'bdsu-services-filter', '', array(), '1.10.6', true );
...
function bdsu_home_hub() {
	$featured = array(
		array( 'linkedin-visibility-amplification-system-for-professionals', 'LinkedIn Visibility', 'Monthly professional authority ΓÇö stay visible to people who already matter. Built around professional visibility.', 'LinkedIn' ),
		array( 'linkedin-viral-posts-for-professionals', 'LinkedIn Content', 'Expert-written posts for attention and credibility, built around professional trust.', 'LinkedIn' ),
		array( 'instagram-viral-growth-discovery-system', 'Instagram Growth', 'Discovery and engagement around your brand. We focus on profile discovery and engagement quality.', 'Social Growth' ),
		...
		array( 'facebook-growth-visibility-campaigns', 'Facebook Visibility', 'Stay visible to a relevant Facebook audience ΓÇö scoped campaigns, built around relevant reach.', 'Social Growth' ),
		array( 'website-seo-audit', 'Website SEO Audit', 'Explainable technical and on-page audit with prioritized fixes ΓÇö no ranking promises.', 'Local & Web' ),
	);
	$cards = '';
	...
	if ( $posts ) {
		foreach ( $posts as $post ) {
			...
			$blog .= '<article class="bdsu-blog-card">' . $media . '<span>' . esc_html( get_the_date( 'M j, Y', $post ) ) . '</span><h3><a href="' . esc_url( $permalink ) . '">' . esc_html( get_the_title( $post ) ) . '</a></h3><p>' . esc_html( $excerpt ) . '</p><a class="bdsu-read" href="' . esc_url( $permalink ) . '">Read article ΓåÆ</a></article>';
		}
	} else {
	...
	<div class="bdsu-home-page" data-bdsu-hub="<?php echo esc_attr( BDSU_VER ); ?>">
		<section class="bdsu-home-hero"><div class="bdsu-home-copy"><div class="bdsu-eyebrow">Marketing systems built around your next goal</div><h1>Grow your brand. Get discovered. Turn attention into revenue.</h1><p>BrandDad Social is local/web/AI ads services and learning ΓÇö LinkedIn, SEO, reputation and PR. Logos and websites live on BrandDad.co. Hosting lives on HostTech.</p><div class="bdsu-hero-actions"><a class="bdsu-primary" href="<?php echo esc_url( bdsu_site_check_url() ); ?>">Check your website</a><a class="bdsu-secondary" href="<?php echo esc_url( bdsu_po_keep( home_url( '/services/' ) ) ); ?>">Explore Marketing Services</a><button class="bdsu-secondary bdsu-quiz-open" type="button">Find My Best Service</button></div><div class="bdsu-trust-row"><span>Γ£ô Free site check</span><span>Γ£ô Clear pricing</span><span>Γ£ô 10% member savings on eligible services</span></div></div><div class="bdsu-home-panel"><span>What do you want to do?</span><a href="<?php echo esc_url( bdsu_site_check_url() ); ?>">Check your website status <b>ΓåÆ</b></a><a href="<?php echo esc_url( bdsu_po_keep( home_url( '/services/' ) ) ); ?>">Buy a marketing service <b>ΓåÆ</b></a><a href="#" class="bdsu-quiz-open">Get a personalized recommendation <b>ΓåÆ</b></a><a href="<?php echo esc_url( bdsu_po_keep( 'https://directory.branddad.social/?utm_source=branddad_social&utm_medium=home' ) ); ?>">Find businesses in the Directory <b>ΓåÆ</b></a></div></section>
		<section class="bdsu-ecosystem"><div class="bdsu-section-intro"><span>Choose your next step</span><h2>Pick the BrandDad door that fits.</h2><p>Growth services live here. Logos, hosting, and WhatsApp Directory are one honest click away.</p></div><div class="bdsu-ecosystem-grid"><article><b>01</b><h3>Growth services</h3><p>LinkedIn, social growth, SEO, reputation, PR, and management ΓÇö buy only what you need.</p><a href="<?php echo esc_url( bdsu_po_keep( home_url( '/services/' ) ) ); ?>">Shop services ΓåÆ</a></article><article><b>02</b><h3>Need a website?</h3><p>Logos and premium websites live on BrandDad.co ΓÇö not here.</p><a href="<?php echo esc_url( bdsu_po_keep( 'https://branddad.co/get-started/?utm_source=branddad_social&utm_medium=home' ) ); ?>" target="_blank" rel="noopener">Start a logo or website ΓåÆ</a></article><article><b>03</b><h3>Need hosting?</h3><p>Domains and hosting live on HostTech ΓÇö where the site lives.</p><a href="<?php echo esc_url( bdsu_po_keep( 'https://hosttech.net/shopping/?utm_source=branddad_social&utm_medium=home' ) ); ?>" target="_blank" rel="noopener">Shop HostTech plans ΓåÆ</a></article><article><b>04</b><h3>Get found on WhatsApp</h3><p>Directory is the WhatsApp business listing. Members save 10% on eligible services here.</p><a href="<?php echo esc_url( bdsu_po_keep( 'https://directory.branddad.social/?utm_source=branddad_social&utm_medium=home&utm_campaign=directory10' ) ); ?>" target="_blank" rel="noopener">Open Directory ΓåÆ</a></article></div></section>
		<?php bdsu_affiliate_promo(); ?>
		<section class="bdsu-home-services bdsu-home-services--shop" aria-labelledby="bdsu-home-services-title">
			<div class="bdsu-section-heading">
				<div>
					...
					<p>LinkedIn, Instagram, Telegram, and Facebook growth systems ΓÇö plus the SEO audit that follows a site check. Clear scope and transparent starting prices.</p>
				</div>
			...
			<p class="bdsu-home-services-foot"><a href="<?php echo esc_url( bdsu_po_keep( home_url( '/services/' ) ) ); ?>">See all services &amp; pricing ΓåÆ</a></p>
		</section>
		...
		<section class="bdsu-final-cta"><div><span>Not sure what to buy?</span><h2>Tell us your goal and weΓÇÖll point you in the right direction.</h2></div><a href="https://wa.me/18729105115?text=Help%20me%20choose%20the%20right%20BrandDad%20service">Chat on WhatsApp</a></section>
	</div>
...
function bdsu_product_value_box() {
	echo '<div class="bdsu-product-value"><strong>Directory members save 10%</strong><span>Active BrandDad Directory members get 10% off eligible services across BrandDad Social, BrandDad.co, and HostTech. If the discount does not apply at checkout, message us with your Directory profile.</span><a href="https://directory.branddad.social/" target="_blank" rel="noopener">Join or verify at directory.branddad.social ΓåÆ</a></div>';
}
...
function bdsu_service_styles() { ?>
	...
	/* High-contrast secondary CTA ΓÇö blue fill + white (survives global .bdsu-primary color:!important) */
	...
	/* 1.10.0 ΓÇö text cards stay image-free; photo cards show AI media */
...
function bdsu_growth_advisor() {
	...
	<div class="bdsu-contact-dock" aria-label="BrandDad help options">
		<div class="bdsu-contact-menu" id="bdsu-contact-menu" hidden>
			...
			<button type="button" class="bdsu-contact-choice bdsu-quiz-open">Find my best service <b>ΓåÆ</b></button>
			<a href="https://affiliates.branddad.social/">Earn 30% recurring <b>ΓåÆ</b></a>
			<a href="https://wa.me/18729105115?text=Help%20me%20choose%20a%20BrandDad%20service">WhatsApp <b>ΓåÆ</b></a>
			<a href="https://www.facebook.com/branddad.social" target="_blank" rel="noopener">Facebook <b>ΓåÆ</b></a>
			<a href="https://www.instagram.com/branddadsocial/" target="_blank" rel="noopener">Instagram <b>ΓåÆ</b></a>
		</div>
		<button type="button" class="bdsu-contact-toggle" aria-expanded="false" aria-controls="bdsu-contact-menu"><span>Need help?</span><b>≡ƒÆ¼</b></button>
	</div>
	<div class="bdsu-quiz-modal" id="bdsu-growth-quiz" hidden>
		...
		<section class="bdsu-quiz-card" role="dialog" aria-modal="true" aria-labelledby="bdsu-quiz-title">
			<button type="button" class="bdsu-quiz-close" data-bdsu-close aria-label="Close">├ù</button>
			<div class="bdsu-quiz-intro"><span>BrandDad Growth Advisor</span><h2 id="bdsu-quiz-title">Find the service that fits you.</h2><p>Answer five quick questions. WeΓÇÖll recommend the most useful next stepΓÇöwithout collecting your information.</p></div>
			<form class="bdsu-quiz-form">
				...
				<label>What budget feels comfortable?<select name="budget" required><option value="">Choose one</option><option value="starter">Under $250</option><option value="growth">$250ΓÇô$999</option><option value="pro">$1,000ΓÇô$2,499</option><option value="scale">$2,500+</option></select></label>
...
function bdsu_growth_advisor_assets() {
	...
	document.addEventListener('DOMContentLoaded',function(){
		...
		form.addEventListener('submit',function(e){e.preventDefault();var d=new FormData(form),goal=d.get('goal'),platform=d.get('platform'),challenge=d.get('challenge'),budget=d.get('budget');var title='Social Media Management',desc='A consistent multi-platform plan will give you the clearest next step.',url='https://branddad.social/product/social-media-management/';
			if(challenge==='website'){title='Website SEO Audit';desc='Start with an explainable site audit, then choose fixes, speed, or conversion work ΓÇö no ranking promises.';url='https://branddad.social/product/website-seo-audit/'}
			...
			else if(platform==='linkedin'){title='LinkedIn Visibility';desc='Build professional authority and a repeatable path to conversations ΓÇö built around professional visibility.';url='https://branddad.social/product/linkedin-visibility-amplification-system-for-professionals/'}
